...
function TradingPairs() {
  ...
  const pairs = robot?.pairs ?? (robot?.symbols || []).map(defaultPair);
  const symbols = Array.from(new Set(robot?.symbols || [])).filter(Boolean);
