...
function AdminConsole() {
  ...
  useEffect(() => {
    ...
    for (const local of storeRef.current.accounts) {
      ...
      const remote = cloud.accounts.find((candidate) => candidate.email.toLowerCase() === local.email.toLowerCase());
      if (remote && remote.status !== local.status) {
        void syncAdminUpdate({ data: { adminEmail: account.email, targetEmail: local.email, patch: { status: local.status } } });
      }
    }
  ...
  // device too (no-op when KV is not configured). A failed write is LOUD:
  // a silently dropped approval used to resurrect "pending" on later polls.
