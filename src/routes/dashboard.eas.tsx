...
function ManageEAs() {
  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-2xl text-sm text-muted-foreground">Create and save your EA profile with its briefing, symbols, picture, and robot video. These owner assets are never shown in the admin console.</p>
...
function CreateEaDialog({ open, onOpenChange, onCreate }: { open: boolean; onOpenChange: (value: boolean) => void; onCreate: (ea: ExpertAdvisor) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border-border/60 bg-card p-6 sm:max-w-md"><div className="flex items-start"><div className="flex-1"><DialogTitle className="text-xl font-bold">Create Expert Advisor</DialogTitle><p className="mt-2 text-sm text-muted-foreground">Save the profile that your clients will see in the app.</p></div><button type="button" onClick={() => onOpenChange(false)} aria-label="Close" className="text-muted-foreground"><X className="size-5" /></button></div>
    <form className="mt-6 space-y-5" onSubmit={(event) => { event.preventDefault(); setError(""); if (!name.trim()) { setError("Enter a codename for your EA."); return; } if (!image || !video) { setError("Choose both a picture and a robot video before deploying."); return; } onCreate({ ...createEaRecord(name), briefing: briefing.trim(), symbols, image, video }); reset(); onOpenChange(false); }}>
      ...
      <div><p className={label}>Robot Video — Shown in the app instead of the image</p><label className="mt-2 flex h-36 cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-border/70 text-sm text-muted-foreground"><Video className="size-6" />{videoName || "Click to upload video (max 3MB)"}<input type="file" accept="video/*" className="hidden" onChange={(event) => chooseFile(event.target.files?.[0], "video")} /></label></div>
