...
function AppAccess() {
  ...
  return <div className="mx-auto min-h-screen w-full max-w-md px-5 py-6 sm:max-w-lg">
    ...
    <main className="flex min-h-[calc(100vh-6rem)] flex-col justify-center py-10">
      {!showLicenseView ? <PricingView email={email} setEmail={setEmail} appEmail={app.email} onContinue={continueWithEmail} checkoutUrl={WHOP_CHECKOUT_URL} /> : <LicenseView email={activeEmail} setEmail={setEmail} keyValue={key} setKey={setKey} onSubmit={submitLicense} admin={paymentStatus === "admin"} paid={paymentStatus === "paid" || successReturn} />}
    </main>
  </div>;
}
