...
function buildChartPlan(signal: ScannerAnalysis["signal"], timeframe: string, referencePrice: string): ChartPlan | null {
  const entry = Number(referencePrice);
  if (!Number.isFinite(entry) || entry <= 0 || signal === "NO TRADE") return null;

  // No live quote is used. The transparent timeframe model gives a fixed 1:2 manual plan.
  const riskRate = timeframe === "15m" ? 0.0012 : timeframe === "4h" ? 0.004 : 0.0025;
  const risk = entry * riskRate;
  const reward = risk * 2;
  return signal === "BUY"
    ? { entry, stopLoss: entry - risk, takeProfit: entry + reward, riskReward: "1 : 2" }
    : { entry, stopLoss: entry + risk, takeProfit: entry - reward, riskReward: "1 : 2" };
}
...
export default function ChartScanner({ symbols, pairs = [], accent, scansLeft, onScanStart }: Props) {
  ...
  const plan = analysis ? buildChartPlan(analysis.signal, analysis.timeframe, referencePrice) : null;
          ...
          {analysis && (
            <>
              ...
              <div className="rounded-[24px] border border-white/5 bg-[#151515] p-4">
                ...
                <div className="mt-4 rounded-[16px] border border-white/10 bg-black/40 p-3">
                  <label className="block text-[10px] font-bold tracking-[0.12em] text-white/45" htmlFor="chart-reference-price">
                    REFERENCE PRICE FROM YOUR CHART
                  </label>
                  <input
                    id="chart-reference-price"
                    type="number"
                    inputMode="decimal"
                    min="0"
                    step="any"
                    ...
                    placeholder="Enter the latest price shown in the screenshot"
                    className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-[#111] px-3 text-sm font-bold text-white outline-none placeholder:text-white/25 focus:border-white/30"
                  />
                  <p className="mt-2 text-[11px] leading-relaxed text-white/40">
                    No live quote is used. This price anchors the manual SL/TP plan from the uploaded chart direction.
                  </p>
                </div>
                <div className="mt-3 flex items-center justify-between rounded-[16px] bg-black px-4 py-3">
                  <span className="text-[10px] tracking-[0.12em] text-white/40">DIRECTION</span>
                  <span className="text-sm font-black" style={{ color: analysis.signal === "BUY" ? "#22c55e" : analysis.signal === "SELL" ? "#ef4444" : "#9ca3af" }}>
                    {analysis.signal === "BUY" ? "LONG · BULLISH" : analysis.signal === "SELL" ? "SHORT · BEARISH" : "WAIT · NO TRADE"}
                  </span>
                </div>
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <div className="rounded-[16px] bg-black p-3 text-center">
                    <p className="text-[10px]" style={{ color: accent }}>ENTRY</p>
                    <p className="mt-1 text-[12px] font-bold text-white">{plan ? fmtPrice(plan.entry) : "—"}</p>
                  </div>
                  <div className="rounded-[16px] bg-black p-3 text-center">
                    <p className="text-[10px] text-red-400">⚠ SL</p>
                    <p className="mt-1 text-[12px] font-bold text-white">{plan ? fmtPrice(plan.stopLoss) : "—"}</p>
                  </div>
                  <div className="rounded-[16px] bg-black p-3 text-center">
                    <p className="text-[10px] text-green-400">TP</p>
                    <p className="mt-1 text-[12px] font-bold text-white">{plan ? fmtPrice(plan.takeProfit) : "—"}</p>
                  </div>
                </div>
                <div className="mt-3 flex items-center justify-between text-[11px]">
                  <span className="text-white/30">RISK : REWARD</span>
                  <span className="font-bold" style={{ color: accent }}>{plan?.riskReward ?? "Enter price"}</span>
                </div>
                <div className="mt-3 space-y-1.5">
                  {analysis.reasons.map((reason, index) => (
                    <p key={index} className="text-[12px] leading-relaxed text-white/50">• {reason}</p>
                  ))}
                  {analysis.signal !== "NO TRADE" && !plan && (
                    <p className="text-[12px] font-bold leading-relaxed text-amber-300">• Enter the latest chart price above to calculate numeric SL and TP.</p>
                  )}
                  {plan && (
                    <p className="text-[12px] font-bold leading-relaxed text-green-300">• Manual plan ready: {plan.riskReward} risk/reward from the chart reference price.</p>
                  )}
                </div>
                ...
                {analysis.signal === "NO TRADE" ? (
                  ...
                  <div className="mt-4 flex min-h-[56px] w-full items-center justify-center rounded-full bg-green-500/10 px-4 text-center text-sm font-bold text-green-300">
                    Use this signal, SL and TP to execute manually
                  </div>
                )
                <button
                  type="button"
